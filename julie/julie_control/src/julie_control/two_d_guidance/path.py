import logging, numpy as np, scipy.signal
import lxml.etree as ET

import pdb
LOG = logging.getLogger('path')

def _parse_2d_pt(txt): return [float(_t) for _t in txt.split(',')]
...
class LegLine(Leg):
    def __init__(self, **kwargs):
        if 'xml_e' in kwargs:
            _pts = kwargs['xml_e'].text.split(' ')
            self._start, self._end = [np.array(_parse_2d_pt(_t)) for _t in _pts]
        self._d_points = 0.1
        _disp = self._end - self._start
        n_pt = np.linalg.norm(_disp)/self._d_points
        self._points = np.stack([np.linspace(self._start[i], self._end[i], n_pt) for i in range(2)], axis=-1)
        self._headings =  np.arctan2(_disp[1], _disp[0])*np.ones(len(self._points))
        self._dist = [] # TODO
        #print self._start, self._end, self._points 
        
class Path:

    leg_class_of_str = {'line': LegLine }
    ...
    def __init__(self, **kwargs):
        ...
        elif 'load_xml' in kwargs:
            self.load_xml(kwargs['load_xml'])
        else:
            self.clear()
    ...
    def load_xml(self, filename):
        self.clear()
        LOG.info(" reading maze from {}".format(filename))
        tree = ET.ElementTree(file=filename)
        xml_path = tree.getroot()
        for e in xml_path:
            if e.tag == 'leg':
                self._legs.append(Path.leg_class_of_str[e.attrib['type']](xml_e=e))
                #print e.text
        self.points = np.concatenate([_l._points for _l in self._legs])
        self.headings = np.concatenate([_l._headings for _l in self._legs])
        self.compute_dists()
        #pdb.set_trace()
